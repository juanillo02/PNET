...
document.addEventListener("DOMContentLoaded", function () {
    ...
    let fechaReservaParaHoras = null;

// Con este evento conseguimos que un mensaje de error se muestre cuando pulsemos un lunes en el calendario (según nuestro horario, ese día no se trabaja :D ). 
// Además, según nuestra política, no se pueden elegir fechas posteriores a una semana desde el día actual. Por lo tanto, con este evento bloqueamos todos los días anteriores al actual y posteriores a una semana.
document.getElementById('calendario').addEventListener('click', function (event) {
    ...
    fechaReserva.addEventListener('input', function () {
        ...
        fechaReservaParaHoras = fechaSeleccionada;
...
for (let opcion of document.querySelectorAll(".Horarioscheck input")) {
    opcion.addEventListener('input', function (event) {
        ...
        // Símbolos para separar el value del input (Ej: value ="11:00-19:00")
        let simboloGuion = "-";
        let simbolo2puntos = ":";
        ...
        // Separamos la hora de los minutos (en la primera hora almacenada: 18)
        var horaInicio = separarHoras[0].split(simbolo2puntos);
        
        if(fechaActual.toISOString().split('T')[0] === fechaReservaParaHoras.toISOString().split('T')[0])
        {
            // Si la hora seleccionada es menor que la hora actual, se trata del día siguiente
            if (parseInt(horaInicio[0]) < horaActual || (parseInt(horaInicio[0]) === horaActual && parseInt(horaInicio[1]) <= minActual)) {
                // Si la hora seleccionada es a medianoche (00:15 por ejemplo) y es menor que la hora actual (12:00),
                // entonces debe ser válida, ya que corresponde al día siguiente
                if (parseInt(horaInicio[0]) >= 0 && parseInt(horaInicio[1]) >= minActual) {
                    inputActual.setCustomValidity('');  // No hay error, ya que corresponde al día siguiente
                } else {
                    inputActual.setCustomValidity('La hora seleccionada es anterior a la actual.');
                    inputActual.reportValidity(); 
                    return; // Sale de la función para evitar continuar
                }
            } else {
                inputActual.setCustomValidity('');  // Eliminamos el mensaje de error si es válida
            }
        } else {
            inputActual.setCustomValidity('');  // Eliminamos cualquier mensaje de error si no es el día actual
        }
    });
}
...
function nombreCorrecto(nombre) {
    let expresionNombre = /[A-Za-z\u00E0-\u00FC]+\s[A-Za-z\u00E0-\u00FC]+\s[A-Za-z\u00E0-\u00FC]+/;
...
function botonesLocalidad() {
    ...
    if (ciudad1) {
        ciudad1.style.backgroundColor = '#5C1C4D'; // Elimina color de fondo (o cualquier estilo)
        ciudad1.style.borderColor = '#774168'; // Elimina el borde (si es necesario)
    }
    ...
    for (let boton of botonesSinAceptar) {
        boton.addEventListener('click', function () {
            ...
            if (boton.textContent === 'Cádiz') {
                ...
                for (let sala of ElPuerto) {
                    sala.style.display = 'block';
                    ciudad1.style.backgroundColor = ''; // Elimina color de fondo (o cualquier estilo)
                    ciudad1.style.borderColor = ''; // Elimina el borde (si es necesario)
                }
